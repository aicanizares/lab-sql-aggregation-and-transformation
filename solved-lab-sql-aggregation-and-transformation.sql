...
SELECT 
    ...
    LEFT(email, 3) AS email_3_characters
FROM customer
ORDER BY last_name ASC;

# Challenge 2. 1. Next, you need to analyze the films in the collection to gain some more insights. Using the film table, determine:
# 1.1 The total number of films that have been released.
SELECT COUNT(*) AS total_films_released 
FROM film;

# 1.2 The number of films for each rating.
SELECT rating, COUNT(*) AS number_of_films_by_rating
FROM film 
GROUP BY rating;
...
FROM film 
GROUP BY rating 
ORDER BY number_of_films_by_rating DESC;
...
# 3. Bonus: determine which last names are not repeated in the table actoSELECT last_name
SELECT last_name
FROM actor
GROUP BY last_name
